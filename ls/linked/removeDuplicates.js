...
prev = 1;
curr = 2;
- Were going to curr.next until curr is not equal to duplicate val
- We then point prev.next to curr;
*/
function removeDuplicates(head) {
    ...
    while (curr !== null && curr.next) {
        if (curr.val === curr.next.val) {
            ...
            while (curr.next && curr.val === duplicate) {
                curr = curr.next;
            }
            curr = curr.val === duplicate ? curr.next : curr;
            prev.next = curr;
        } else {
            prev = curr;
            curr = curr.next;
